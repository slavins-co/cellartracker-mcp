...
- [x] #54 slice B — Pagination: `offset` param added to search-cellar/consumption-history/tasting-notes/bottle-details; text footer "(Showing X-Y of Z. Pass offset=Y…)" shown only when more results remain; an offset past the end of a non-empty result set gets a distinct message instead of the misleading zero-match text. Fixed a real bug caught by review: consumption-history/tasting-notes were missing bottle-details' non-positive `max_results` guard (extracted to shared `clampLimit()`). (PR #88)
- [x] #54 slice C — MCP resources: `cellartracker://tables/<Table>` CSV resource per table (8 total) + `cellartracker://meta/cache` JSON resource with per-table freshness + server version; cellartracker-data skill updated to mention resources. Design change from the original plan, made during review: meta/cache stats the cache dir directly instead of routing through `ensureFresh` — reading it no longer requires credentials or can trigger a full network re-export just to report freshness. Completes #54 (all 4 items: annotations #77, structured output #87, pagination #88, resources #89). (PR #89)
...
- [ ] Follow-up (from #54 slice B, PR #88): the deferred `listPayload()`/`renderResultList()` helper (first flagged in #49, again in #54 slice A) surfaced a 3rd time — slice B added offset-aware slicing + a 3-branch response shape to all 4 capped tools, still hand-duplicated per tool beyond the two small helpers (`clampOffset`/`clampLimit`/`paginationFooter`) that were extracted this round. drinking-recommendations also still has an unpaginated, unconverted version of the same truncate-and-render shape. Next touch to any of these 5 handlers is the trigger point to stop deferring and land the shared helper.
- [ ] Follow-up (from #54 slice C, PR #89): `exporter.ts`'s `saveCsv()` overwrites `<Table>_latest.csv` via a non-atomic `fs.copyFileSync`, so a concurrent read (any tool, or a new resource read) during a `refresh-data` call has a narrow theoretical window to observe a partially-written file. Pre-existing across every read path since before slice C, not introduced by it, flagged during slice C's review — low real-world risk (single-user local server, small CSVs) but the correct fix if ever prioritized is write-then-atomic-rename instead of write-then-copy.
- [x] #90 — CI: add workflow-root `permissions: contents: read` to ci.yml, resolving CodeQL `actions/missing-workflow-permissions` alerts 1 and 2 (PR #91)
