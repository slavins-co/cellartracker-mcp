...
- `foldDiacritics()` (`s.normalize("NFD").replace(/\p{M}/gu, "").toLowerCase()`) only strips diacritics that have a canonical base-letter + combining-mark decomposition (é→e+´, ô→o+^, ü→u+¨) — it does nothing for standalone Unicode letterforms with no such decomposition: ø, æ, ł, đ, ß, ligatures. A query like "strasse" won't match data containing "Straße" (e.g. the German wine region "Deutsche Weinstraße"). This matches issue #48's exact prescribed fix and acceptance criteria (which only cite NFD-decomposable examples: Rhône/Côte/Grüner), so it's not a defect in that implementation — but if diacritic-folding is extended or reused elsewhere, closing this gap needs an explicit substitution map, not a smarter regex. (Issue #48, PR #75)
